export default async function getCommercialOperations(date,province,page=1){
  ...
  try {
    let config = {
      ...
      let res = await fetch(`http://localhost:8000/reportes/CommercialOperations/?date=${date}&commercial_office__office_province=${province}&page=${page}`, config)
      data = await res.json()
      console.log(data)
      if(res.status == 401){
        console.log("401")
