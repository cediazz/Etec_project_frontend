export default async function getCommercialOfficeProvince(){
    ...
    try {
      let config = {
        ...
        let res = await fetch(`http://127.0.0.1:8000/reportes/CommercialOfficeProvinces/`, config)
        data = await res.json()
        console.log(data)
        if(res.status == 401){
          console.log("401")
          ...
          let res2 = await fetch(`http://127.0.0.1:8000/reportes/CommercialOfficeProvinces/`, config)
